...
def show_my_pools(db, current_profile):
    """Show pools the user is participating in."""
    ...
    for participant in participants.data:
        # Get pool details
        pool_result = db.table('pools').select("*").eq(
        ...
        with st.expander(f"🏈 {pool['pool_name']} ({pool['season_year']})"):
            ...
            with col2:
                ...
                st.write(f"**Teams Selected:** {'✅ Yes' if participant['selections_complete'] else '❌ No'}")
            ...
            if selections.data:
                ...
                for selection in sorted(selections.data, key=lambda x: x['selection_order']):
                    # Get team info
                    team_result = db.table('nfl_teams').select("*").eq(
                    ...
                    if team_result.data:
                        ...
                        teams.append(f"{selection['selection_order']}. {team['team_city']} {team['team_name']} ({team_points} pts)")
                    else:
            ...
            col1, col2 = st.columns(2)
            with col1:
                if not participant['selections_complete']:
                    if st.button(f"Select Teams", key=f"select_{pool['pool_id']}"):
                        st.info("Go to 'Team Selection' page to pick your teams")
            
            with col2:
                if st.button(f"View Leaderboard", key=f"leader_{pool['pool_id']}"):
                    st.info("Go to 'Leaderboards' page to see rankings")


def show_join_pool(db, current_profile):
    """Show available pools to join."""
    ...
    my_pools = db.table('pool_participants').select("pool_id").eq(
        'profile_id', current_profile['profile_id']
    ).execute()
            ...
            with col3:
                if participant_count < pool['max_participants']:
                    if st.button("Join Pool", key=f"join_{pool['pool_id']}"):
                        ...
                        result = db.table('pool_participants').insert(participant_data)
...
def show_create_pool(db):
    ...
    with st.form("create_pool_form"):
        ...
        with col1:
            season_year = st.number_input("Season Year", 
        ...
        with col2:
            max_participants = st.number_input("Max Participants", 
                                             min_value=2, 
                                             max_value=100, 
                                             value=20)
            deadline_days = st.number_input("Registration deadline (days from now)", 
                                          min_value=1, 
                                          max_value=30, 
                                          value=7)
        ...
        if submitted:
            if not pool_name:
                ...
                result = db.table('pools').insert(pool_data)
                ...
                if result.data:
                    ...
                    db.table('pool_participants').insert(participant_data)
                    
                    st.success(f"Pool '{pool_name}' created successfully! You've been automatically added as a participant.")
                    st.rerun()
                else:
