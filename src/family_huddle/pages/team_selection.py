...
def show(db: "Client") -> None:
    """Display the team selection interface for pools.

    Allows users to select exactly 4 NFL teams for each pool they've joined.
    Teams are organized by conference and division with point values displayed.
    ...
    current_participant = next(
        (p for p in participants.data if p["pool_id"] == selected_pool["pool_id"]), None
    )
    selections_complete = current_participant and current_participant.get(
        "selections_complete", False
    )

    if selections_complete:
        st.info(
            "✅ You have completed your team selections for this pool. You can modify them below if needed."
        )
    else:
    ...
    selected_team_ids = (
        [s["team_id"] for s in current_selections.data] if current_selections.data else []
    )
    ...
    conferences: dict[str, dict[str, list[dict[str, Any]]]] = {"AFC": {}, "NFC": {}}
    ...
    with col1:
        if st.button("Clear Selections", type="secondary", use_container_width=True):
            db.table("team_selections").delete().eq("pool_id", selected_pool["pool_id"]).eq(
                "profile_id", current_profile["profile_id"]
            ).execute()

            db.table("pool_participants").update({"selections_complete": False}).eq(
        ...
        if st.button(save_button_text, type="primary", use_container_width=True):
            ...
            if len(all_selected) != 4:
                ...
                for i, team_id in enumerate(all_selected):
                    ...
                    db.table("team_selections").insert(selection_data).execute()

                db.table("pool_participants").update({"selections_complete": True}).eq(
                    "pool_id", selected_pool["pool_id"]
                ).eq("profile_id", current_profile["profile_id"]).execute()
...
def show_conference_teams(
    db: "Client",
    conference: str,
    divisions: dict[str, list[dict[str, Any]]],
    already_selected: list[Any],
    selected_teams: list[Any],
) -> None:
    """Display teams for a conference organized by division with selection interface.

    Creates multiselect widgets for each division within the conference,
    pre-selecting already chosen teams.

    Args:
        db: Supabase client for database operations.
        conference: Conference name (AFC or NFC).
        divisions: Dictionary mapping division names to team lists.
        already_selected: List of team IDs already selected by user.
        selected_teams: List to accumulate newly selected team IDs.
    """
